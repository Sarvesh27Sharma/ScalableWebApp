...
public class ScalableWebAppServiceImpl implements ScalableWebAppService {
    ...
    @Override
    public boolean saveLeftSideData(DifferedDTO differedDTO) {
        Optional<DifferedEntity> optionalDifferedEntity = repository.findById(differedDTO.getId());
        DifferedEntity entity;
        if (optionalDifferedEntity.isPresent()) {
            entity = optionalDifferedEntity.get();
            entity.setLeft(differedDTO.getLeft());
        } else {
            entity = mapper.mapDifferedDTOToEntity(differedDTO);
        }
    ...
    @Override
    public boolean saveRightSideData(DifferedDTO differedDTO) {
        Optional<DifferedEntity> optionalDifferedEntity = repository.findById(differedDTO.getId());
        DifferedEntity entity;
        if (optionalDifferedEntity.isPresent()) {
            entity = optionalDifferedEntity.get();
            entity.setRight(differedDTO.getRight());
        } else {
            entity = mapper.mapDifferedDTOToEntity(differedDTO);
        }
        log.info("entity to be persist :{}", entity);
    ...
    public ApiResponseDTO getById(Long id) throws ScalableWebApiException {
        ...
        if (optionalDifferedEntity.isPresent()) {
            log.info("optionalDifferedEntity.get() :{}", optionalDifferedEntity.get());
    ...
    private void inValidDifferedDTO(DifferedDTO differedDTO) throws ScalableWebApiException {
        log.info("differedDTO :{}", differedDTO);
        ErrorsDTO errorsDTO = new ErrorsDTO();
    ...
    /**
     * method to Calculate diff for successful validated request
     *
     ...
     * @return status for the diff-ed operation
     */
